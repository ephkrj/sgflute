...
struct Person {
 int gender ,race ,citizen ,marital ,number_children ,nSchoolType ,qualification ,work ,income ,headhouse ,occupation ,religion ,partnerid ,houseid ,dwelling ,hasmaid ,mobility , nhours ,industry ,NS ,trans_mode ,trans_time ,oversea ,housesize ,f_nuclei ,generation ,youngest ,numwork , hincome ,momid ,dadid ,unit_id,exactAge;
  ...
  //Home
  int nHomeTract;	      // (zone_home)
  int nHomeComm;     // ID of home community (subzone_home)		removed unsigned
  int nHomeNeighborhood;    // (postcode_home) **(change from char)	removed unsigned
  ...
  //Work
  int nDayTract;        // ID of daytime (work, school) tract (zone_work)
  int nDayComm;         // ID of daytime (work, school) community (subzone_work)
...
struct Community {
  ...
  std::list <Person> visitors;           	// list of short-term travelers to this community

  // Initialized and fixed
  int id;	 		// unique community ID      removed unsigned
  int nTractID;          		// ID of its tract
  unsigned int nFirstPerson, nLastPerson; // ID of first and last person(+1) in community

  // Initialized but updated during run-time
  ...
  int nNumWorkersLeaving;		// number of residents who work outside the community, PROBABLY NEVER USED! VERIFY THIS!
  ...
  // Contact rates adjusted in run-time. Trace this!
...
struct Neighborhood {
  const static int TARGETNEIGHBORHOODSIZE=500;

  // Initialized and fixed 
  int id;	 		// unique neighborhood ID	removed unsigned
  int nTractID;          		// ID of its tract
  int nCommID;          		// ID of its community
...
struct Tract {
  // Initialized and fixed
  int id;          		// unique tract identifier      removed unsigned
...
class EpiModel {
 ...
 protected:
  ...
  unsigned int nNumNeighborhood;    // number of neighborhoods on this node
  ...
  vector < Neighborhood > nhvec;    // the neighborhoods
  map<int, int> postaltonh; 		// resolve postal codes to id in the nhvec
