...
//creating Neighborhoods/postalcodes from ider
	for(size_t i=0,max=ider.size();i!=max;i++){			
		...
		n.nNumWorkers=0;
		n.nNumNonWorkers=0;
		for(std::vector<Person>::iterator it=pvec.begin(),end=pvec.end();it!=end;it++){		//counting
			Person &p=*it;
...
//creating Communities/subzones from ider
	...
	for(size_t i=0,max=ider.size();i!=max;i++){
		...
		sort(cwkgrp.begin(),cwkgrp.end());
		cwkgrp.erase(unique(cwkgrp.begin(),cwkgrp.end()),cwkgrp.end());
		c.nNumWorkGroups=cwkgrp.size();					//Need to rethink this part
		//Assigning community id to neighborhood using temp vector
		for(size_t j=0,Max=temp.size();j!=Max;j++){
			for(std::vector<Neighborhood>::iterator it=nhvec.begin(),end=nhvec.end();it!=end;it++){	
				Neighborhood &n=*it;
				if(n.id==temp[j])
					n.nCommID=ider[i];
			}
...
//Creating Tracts/zones from ider
	...
	for(size_t i=0,max=ider.size();i!=max;i++){
		...
		for(size_t j=0,Max=temp.size();j!=Max;j++){
			for(std::vector<Community>::iterator it=commvec.begin(),end=commvec.end();it!=end;it++){
				Community &c=*it;
				if(c.id==temp[j]){
					c.nTractID=ider[i];
	...
	for (vector<Community>::iterator it = commvec.begin(); it != cend; it++) {
		Community &comm = *it;
		double nightscale = Community::TARGETCOMMUNITYSIZE
				/ (double) comm.nNumResidents; // nighttime population
		double dayscale = Community::TARGETCOMMUNITYSIZE
		...
		//cout<<"Comm id: "<<comm.id<<" cpcm:"<<comm.cpcm[0] <<" cpnh: "<<comm.cpnh[0]<<" daycpcm:"<<comm.daycpcm[0] <<" daycpnh: "<<comm.daycpnh[0]<<endl;
		//doing nNumWorker
	...
	for (vector<Community>::iterator it = commvec.begin(); it != cend; it++) {
		...
		cout<<comm.id<<","<<comm.nNumResidents<<","<<comm.nNumNonWorkers<<","<<comm.nNumWorkers<<endl;
	}
...
void EpiModel::infect(Person& p) {
	...
	if (p.nTravelTimer <= 0) {
		...
		{
#endif
			++commvec[p.nHomeComm-114].ninf[p.age];
			++commvec[p.nHomeComm-114].nEverInfected[p.age];//note the change of -114 (This only works for the Newton input file)
		}
	}
}
...
void EpiModel::TAP(Person& p) {
	...
	enum antiviralPolicy policy = getAVPolicy(
			tractvec[c.nTractID - nFirstTract-26]);
...
void EpiModel::dayinfectsusceptibles(const Person &infected, Community &comm) {
	...
	bool bInfectedAtHome = (isWithdrawn(infected) || isQuarantined(infected)
			...
			|| (isChild(infected) && infected.nWorkplace ==-1
					&& isSchoolClosed(
							tractvec[infected.nDayTract - nFirstTract-26],
	...
	if ((infected.age == 1 || (infected.age == 0 && infected.nWorkplace <0))
			&& isSchoolClosed(tractvec[infected.nDayTract - nFirstTract-26],
					infected.nWorkplace))
	...
	for (unsigned int pid2 = comm.nFirstPerson; pid2 < comm.nLastPerson;
		...
		if (isSusceptible(p2) && p2.nDayComm == comm.id
			...
			if (infected.family == p2.family
					...
					&& (isQuarantined(p2) || p2.nWorkplace == 0
							|| (isChild(p2) && p2.nWorkplace != -1
									&& isSchoolClosed(
											tractvec[comm.nTractID - nFirstTract-26],
			...
			if (casualmultiplier2 == 1.0
					...
					&& isSchoolClosed(tractvec[p2.nDayTract - nFirstTract-26],
							p2.nWorkplace))
	...
	for (vector<unsigned int>::iterator it = comm.workers.begin(); it != comm.workers.end();
			it++) {
		Person &p2 = pvec[*it];
			...
			if (!isWithdrawn(infected) && !isQuarantined(infected)
				...
				if (!infect(p2, infected, comm.cpcm[p2.age],
					...
					if (infected.nHomeNeighborhood == p2.nHomeNeighborhood) // transmission within neighborhood
						if (infect(p2, infected, comm.cpnh[p2.age],
			...
			if (isInfected(p)) {
				if (isInfectious(p)) {
					if (getWillBeSymptomatic(p)
							&& p.iday == getIncubationDays(p)) {
						setSymptomatic(p);
						++commvec[p.nHomeComm-114].nEverSymptomatic[p.age];
						++commvec[p.nHomeComm-114].nsym[p.age];
					}
					if (isSymptomatic(p)) {
						if (p.iday == getIncubationDays(p) + nAscertainmentDelay
								&& getWillBeAscertained(p)) { // person becomes ascertained
							++commvec[p.nHomeComm-114].nEverAscertained[p.age];
		...
		if (p.nTravelTimer <= 0 && !isQuarantined(p)
...
#else
			{
				...
				unsigned int destinationcomm = tractvec[destinationtract
						- nFirstTract-26].nFirstCommunity;
				int diff =
						tractvec[destinationtract - nFirstTract-26].nLastCommunity
								- tractvec[destinationtract - nFirstTract-26].nFirstCommunity;
				if (diff > 1)
			...
			if (j!=rank) {
				...
				if (inflow>0 || outflow>0) {
					...
					for (int i=0; i<inflow; i++) {
						...
						// assign traveler to a community in tract
						traveler.nDayComm = tractvec[traveler.nDayTract-nFirstTract-26].nFirstCommunity;
						int diff = tractvec[traveler.nDayTract-nFirstTract-26].nLastCommunity-tractvec[traveler.nDayTract-nFirstTract-26].nFirstCommunity;
						if (diff>1)
				...
				if (inflow>0 || outflow>0) {
					...
					for (int i=0; i<inflow; i++) {
						...
						if (!isInfected(p) && (pdat->status&INFECTED)) {
							...
							assert(p.age<TAG);
							++commvec[p.nHomeComm-114].ninf[p.age];
							++commvec[p.nHomeComm-114].nEverInfected[p.age];
							p.nInfectedTime = nTimer;
						}
	...
	if (bIndividualsFile) {
		...
		for (vector<Person>::iterator it = pvec.begin(); it != pvec.end();
			...
			<< p.id << "," << (int) p.age << "," << p.family << ","
					...
					<< (int) isVaccinated(p) <<","<<p.x_coor_home<<","<<p.y_coor_home<< endl;
		}
